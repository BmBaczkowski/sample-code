...
This tutorial is based on the tutorial provided by [Hanneke den Ouden](http://www.hannekedenouden.ruhosting.nl/RLtutorial/Instructions.html).
...
Prepare a function to simulate the evolution of values for each of the two stimuli (A, B) over 40 trials in a reversal learning task, given fixed learning rate ($\alpha$) and temperature parameters ($\beta$).
...
Run bootstrapping samples to see 95% interval of the evolution of value for each stimulus in the task given the fixed parameters. 
...
Check whether it is possible to recover the underlying parameters from a single dataset. 
...
We will use prior distribution of the temperature paramter using gamma probability distribution *gamma(2,3)*.
